  ...
  return (
    ...
    <View style={styles.header}>
        ...
        >
          ...
          >
            <Text style={[styles.headerTitle, { opacity: 0 }]}> DOUBLE ENCODER</Text>
          </LinearGradient>
        </MaskedView>
      </View>
    ...
    <View>
      ...
      • Double encoding refers to the act of applying URL encoding (also known as percent-encoding) multiple times to a string. URL encoding is a way to represent special characters in a URL by replacing them with a "%" sign followed by their hexadecimal representation.
For example, the character <Text>{'\u003C'}</Text> is represented as "%3C" when URL encoded.{"\n"}
{"\n"}
Example:{"\n"}
Double-encoded String: %2541%253c%253f%253c%255c%253f 
Decoded String: A<Text>{'\u003C'}</Text> ?<Text>{'\u003C'}</Text> \?
      </Text>
    </View>
    ...
    <Text style={styles.heading}>Double Decoder:</Text>
      <Text style={styles.description}>
      To perform double encoding on an input string, we can utilize a Python program. First, we prompt the user to enter a string they want to double encode. The program takes this input and applies URL encoding twice to the string using the urllib.parse.quote function.
      Checking Double Encoding 
      {"\n"}
      {"\n"}
Scanning for double encoding involves checking if an input string has undergone multiple layers of encoding.
        </Text>
        <View style={styles.outputContainer}>
        <Text style={styles.label}>Double Encoded String:</Text>
      ...
      <View style={styles.outputContainer}>
        ...
        <Text style={styles.output}>
          {isDoubleEncoded ? "The entered string is double encoded." : "The entered string is not double encoded."}
        </Text>
      </View>
    </ScrollView>
